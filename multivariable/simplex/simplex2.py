...
def generate_simplex(X0, alpha):
     ...
     # Create an (N+1) x N matrix to hold the simplex vertices
     X_simplex = np.tile(X0, (N + 1, 1)).astype(float)
     
     # P0 is the initial point X0
     # P_i = X0 + delta1*e_i + delta2*(all other e_j)
     # This loop creates P1 to PN
     for i in range(N):
        # Add delta2 to all coordinates of the i-th point (after P0)
        X_simplex[i + 1, :] += delta2
        # Add the extra bit for the i-th coordinate
...
def centroid(X_except_h):
     ...
     """
     # axis=0 computes the mean down each column (i.e., for each coordinate)
     return np.mean(X_except_h, axis=0)

def reflex(Xc, Xh):
     """
     Reflects the worst point (Xh) through the centroid (Xc).
     alpha_nm (reflection coefficient) is fixed at 1.0.
     Xr = Xc + 1.0 * (Xc - Xh)
     """
     return 2 * Xc - Xh
...
def simplex(x0, alpha, beta, gamma, epsilon, f, max_iter=1000):
     ...
     # 1. Generate initial simplex and evaluate function at each vertex
     ...
     for iter_count in range(max_iter): # Do while with max iterations 
          # 2. Sort vertices by function value (lowest to highest)
          order = np.argsort(F)
          F = F[order]
          X = X[order]
          ...
          # 4. Compute centroid (excluding the worst point Xh)
          Xc = centroid(X[:-1])
          Fc = f(Xc)
...
if __name__ == "__main__":
     ...
     # Set parameters
     alpha_p = 1.0     # Initial simplex size
     beta_p = 0.5    # Contraction
     gamma_p = 3     # Expansion
     epsilon_p = 0.003  # Tolerance

     # --- Test 1 ---
     x_init1 = np.array([-20, -10])
